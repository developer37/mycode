...
// to offer our user
func divideoperation(a, b, c int) {
    // wrap the initial value
...
func main() {
    // run divide
    divideoperation(100,2,2) // should return 25
    
    // run divide by zero
    divideoperation(100,0,20)  // this causes an error
                      // but our handler has us at the same spot
    
    // run divide by zero (again)
    divideoperation(100,5,0)   // we are still handling our errors, just in a highly
                      // abstracted way
}
